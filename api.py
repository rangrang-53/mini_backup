...
async def read_answer():
    return FileResponse("static/html/answer.html")
@app.post("/stt")
...
@app.post("/generate_f_friendly_response")
async def generate_f_friendly_response_endpoint(request: DetailedAnalysisRequest):
    """F 성향 상대를 위한 실천팁과 대안답변 생성 API"""
    try:
        # 이미 구현된 generate_f_friendly_response 함수 호출
        friendly_response = generate_f_friendly_response(request.question, request.answer, request.score)
        
        return {
            "response": friendly_response,
            "friendly_response": friendly_response,  # 호환성을 위해 두 필드 모두 제공
            "success": True
        }
    except Exception as e:
        print(f"❌ F 친화적 응답 생성 오류: {str(e)}")
        raise HTTPException(status_code=500, detail=f"F 친화적 응답 생성 실패: {str(e)}")
