...
async function submitAnswer() {
    ...
    try {
        ...
        const questionElement = document.getElementById('question_text');
        const currentQuestion = questionElement ? questionElement.textContent : '';
        ...
        // localStorage에 현재 질문과 답변 저장 (answer.js에서 사용)
        ...
        // 모든 질문을 완료했는지 확인
        if (currentCount >= maxCount) {
            // 모든 질문 완료 - localStorage에 결과 저장 후 결과 페이지로 이동
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 제출 버튼 클릭 시 answer.html로 이동
    const submitBtn = document.getElementById('user_input_button');
    if (submitBtn) {
